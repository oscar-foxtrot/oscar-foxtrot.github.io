document.addEventListener('DOMContentLoaded', function() {
    const androidRobot = document.getElementById('android-robot-div');
    ...
    function init() {
        ...
        spheres.push(new Sphere(canvas.width / 2, canvas.height - 290, 60, 0, 0, 0));  <!-- CHANGE CHANGE CHANGE -->
    }
    ...
    function showNewSpheres() {
        ...
        fadeInInterval = setInterval(function () {
            ...
            if (!isOverlapping(androidRobot, cont1)) {
                spheres[spheres.length - 1].opacity += 0.04;
            }
            if (allVisible) {
                clearInterval(fadeInInterval);
    ...
    function checkOverlap() {
        if (isOverlapping(androidRobot, cont1)) {
            androidRobot.style.opacity = 0; // Hide the container
            androidRobot.style.pointerEvents = 'none'; // Disable pointer events
        } else {
            androidRobot.style.opacity = 1; // Show the container
            androidRobot.style.pointerEvents = 'auto'; // Enable pointer events
        }
    }
